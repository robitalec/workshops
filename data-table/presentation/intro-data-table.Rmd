...
date: "`r paste0('November 16 2017 [updated: ', paste(month.name[data.table::month(Sys.Date())], data.table::mday(Sys.Date()), data.table::year(Sys.Date())), ']')`"
output: 
...
This avoids the major memory cost of assigning and reassigning objects by changing them in place. 
...
```{r, echo=FALSE}
pander::pander(data.table(
`function` = c('setDT', 'as.data.table', 'data.table'),
description = c('coerce by reference lists or DFs', 
                'cast to data.table (requires assignment)',
                'general constructor')
))
```
...
Notice how the class is still data.frame.

This allows data.tables to (usually) go anywhere a data.frame can.
...
the only requirement for j is to return a list, list of lists or data.frame/data.table. ie: no SpatialPolygons, etc. 
...
```{r}
diamonds[1:3, .(carat, cut, color, clarity, depth, price, meanPrice, meanCarat)]
```
...
syntax for multiple columns to group by:
...
We can also group by a conditional statement
...
If we had column names preassigned, we can pass those to the by argument as well. 
...
...if statements on the value of the group...
...
if multiple by columns, simply select the .BY of interest
...
build your expressions stepwise, ensuring each is working before adding any columns to the DT. 
