...
date: "`r paste0('November 2, 2017 [updated: ', paste(month.name[data.table::month(Sys.Date())], data.table::mday(Sys.Date()), data.table::year(Sys.Date())), ']')`"
output: 
  xaringan::moon_reader:
    nature:
      highlightStyle: github
      beforeInit: 'intro-ggplot/macros.js'
      highlightLines: true
      slideNumberFormat: |
    ...
    css: [metropolis, metropolis-fonts, 'intro-ggplot/extra.css']
---
...
![](images/ggplot2-tutorial-slides.021.png)
...
```{r}
pander::pander(diamonds[1:5, .(cut_color = paste(cut, color, sep="_"),
             price)])
...
```{r}
pander::pander(dcast(diamonds, price ~ cut, value.var = 'z', fun.aggregate = length)[1:5])
```


## The solution: `data.table::melt()` and `data.table::dcast`
...
pander::pander(melt(DT, measure.vars = colnames(DT)[-1], variable.name = 'Cut',
...
## `geom_*`
<!-- This is the type of graph created.
     Simple as points vs lines.
     Or histograms.
     Or....  -->
...
`Worksheet-IntroductionToGgplot.R` 
